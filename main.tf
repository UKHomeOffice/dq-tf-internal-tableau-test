...
#   associate_public_ip_address = false
#   subnet_id                   = "${aws_subnet.subnet.id}"
#   private_ip                  = "${var.dq_tableau_wip_dashboard_instance_ip}"
#   monitoring                  = true
#
#   user_data = <<EOF
# #!/bin/bash
...
# touch /home/tableau_srv/env_vars.sh
# echo "
# export TABLEAU_ENVIRONMENT=wip
# export S3_HTTPD_CONFIG_BUCKET=${var.s3_httpd_config_bucket}
# export DATA_ARCHIVE_TAB_BACKUP_URL=`aws --region eu-west-2 ssm get-parameter --name data_archive_tab_int_backup_url --query 'Parameter.Value' --output text`
# export TAB_INT_REPO_PROTOCOL=`aws --region eu-west-2 ssm get-parameter --name tab_int_repo_protocol --query 'Parameter.Value' --output text`
# export TAB_INT_REPO_USER=`aws --region eu-west-2 ssm get-parameter --name tab_int_repo_user --query 'Parameter.Value' --output text`
...
#
# echo "#TSM settings (add default)"
# export CLIENT_ID=`aws --region eu-west-2 ssm get-parameter --name tableau_wip_openid_provider_client_id --query 'Parameter.Value' --output text`
# export CLIENT_SECRET=`aws --region eu-west-2 ssm get-parameter --name tableau_wip_openid_client_secret --query 'Parameter.Value' --output text --with-decryption`
# export CONFIG_URL=`aws --region eu-west-2 ssm get-parameter --name tableau_wip_openid_provider_config_url --query 'Parameter.Value' --output text`
# export EXTERNAL_URL=`aws --region eu-west-2 ssm get-parameter --name tableau_wip_openid_tableau_server_external_url --query 'Parameter.Value' --output text`
# export TAB_VERSION_NUMBER=`echo $PATH | awk -F customer '{print $2}' | cut -d \. -f2- | awk -F : '{print $1}'`
# cat >/opt/tableau/tableau_server/packages/scripts.$TAB_VERSION_NUMBER/config-openid.json <<EOL
# {
#   "configEntities": {
...
#
#   tags = {
#     Name = "ec2-wip-${local.naming_suffix_wip_linux}"
#   }
#
#   lifecycle {
#     prevent_destroy = true
